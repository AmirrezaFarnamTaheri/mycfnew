...
- `worker_source.js`: نسخهٔ سورس (رفتار یکسان با `worker.js`)
- `worker_obfuscated.js`: نسخهٔ مبهم‌سازی‌شده (سخت‌خوان، ولی رفتار یکسان)
- `worker_source.broken.js`: **استفاده نکنید** (قدیمی/خراب)
...
نکته: فرایند تبدیل در داشبورد از `REMOTE_CONFIG_URL` داخلی (هاردکد در `worker_source.js`) استفاده می‌کند؛ برای تغییر باید همان ثابت را ویرایش کنید.
