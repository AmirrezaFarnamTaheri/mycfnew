    ...
    let backupIPs = [
        ...
        { domain: 'ts.hpc.tw', region: 'TW', regionCode: 'TW', port: 443 }
    ];
    ...
    export default {
        async fetch(request, env, ctx) {
            ...
            if (tjControl !== undefined && tjControl !== '') {
                ...
                const legacyIPsControl = getConfigValue('eli', env.eli);
                if (legacyIPsControl !== undefined && legacyIPsControl !== '') {
                    eli = legacyIPsControl === 'yes' || legacyIPsControl === true || legacyIPsControl === 'true';
                }
            ...
            if (url.pathname === '/doh') return serveLandingPage(request);
    ...
    async function handleSubscriptionRequest(request, user, url = null) {
        ...
        if (eli) {
             const legacyNodes = LEGACY_IPS.map(item => ({
                ip: item.ip,
                port: item.port,
                isp: 'Visa-' + item.type.toUpperCase()
            }));
            await addNodesFromList(legacyNodes);
        }
        ...
        switch (target.toLowerCase()) {
            case 'clash-local':
                subscriptionContent = generateLocalClashConfig(finalLinks, user);
                contentType = 'text/yaml; charset=utf-8';
                break;
            case 'singbox':
            case 'singbox-local':
                subscriptionContent = generateLocalSingboxConfig(finalLinks, user);
        ...
        if (langFromCookie === 'fa' || langFromCookie === 'fa-IR') {
            ...
            const translations = {
                zh: {
                    ...
                    enableLegacyIPs: '启用 Visa/CDN 节点 (eli)',
                ...
                fa: {
                    ...
                    enableLegacyIPs: 'فعال‌سازی نودهای Visa/CDN (eli)',
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="card">
                <h2 class="card-title">Integrations & Tools</h2>
                <div style="margin: 15px 0;">
                    <a href="/doh" target="_blank" style="color: #00ff00; text-decoration: none; margin-right: 20px; font-size: 1.1rem; border-bottom: 1px dashed #00ff00;">👉 Open DoH Proxy Dashboard</a>
                </div>
                <div style="margin: 15px 0;">
                    <label style="display: block; margin-bottom: 8px; color: #00ff00; font-weight: bold;">Subscription Backend (Client Target):</label>
                    <select id="subBackend" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                        <option value="remote">Remote (url.v1.mk) - Default</option>
                        <option value="clash-local">Local Clash (Standalone)</option>
                        <option value="singbox-local">Local Sing-box (Standalone)</option>
                    </select>
                    <small style="color: #00aa00; font-size: 0.85rem;">Select which backend to use for generating subscription configurations.</small>
                </div>
            </div>
                            ...
                            <div style="padding: 15px; background: rgba(0, 20, 0, 0.6); border: 1px solid #00ff00; border-radius: 5px;">
                                ...
                                <div style="margin-bottom: 10px;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00ff00;">
                                        ...
                                        <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00ff00;">
                                            <input type="checkbox" id="eli" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                            <span style="font-size: 1.1rem;">${t.enableLegacyIPs}</span>
                                        </label>
                                </div>
        ...
        <script>
            ...
            async function loadCurrentConfig() {
                ...
                try {
                    ...
                    document.getElementById('eli').checked = config.eli === 'yes' || config.eli === true || config.eli === 'true';
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (advancedConfigForm) {
                    advancedConfigForm.addEventListener('submit', async function(e) {
                        e.preventDefault();
                        const configData = { scu: document.getElementById('scu').value, epd: document.getElementById('epd').checked ? 'yes' : 'no', epi: document.getElementById('epi').checked ? 'yes' : 'no', eli: document.getElementById('eli').checked ? 'yes' : 'no', egi: document.getElementById('egi').checked ? 'yes' : 'no', ae: document.getElementById('apiEnabled').value,
    ...
    function updateConfigVariables() {
        ...
        const legacyIPsControl = getConfigValue('eli', '');
        if (legacyIPsControl !== undefined && legacyIPsControl !== '') {
            eli = legacyIPsControl === 'yes' || legacyIPsControl === true || legacyIPsControl === 'true';
        }
...
const LEGACY_IPS = [
    { ip: 'www.visa.com', port: 80, type: 'http' },
    { ip: 'cis.visa.com', port: 80, type: 'http' },
    { ip: 'africa.visa.com', port: 80, type: 'http' },
    { ip: 'www.visa.com.sg', port: 80, type: 'http' },
    { ip: 'www.visaeurope.at', port: 80, type: 'http' },
    { ip: 'www.visa.com.mt', port: 80, type: 'http' },
    { ip: 'qa.visamiddleeast.com', port: 80, type: 'http' },
    { ip: 'usa.visa.com', port: 443, type: 'https' },
    { ip: 'myanmar.visa.com', port: 443, type: 'https' },
    { ip: 'www.visa.com.tw', port: 443, type: 'https' },
    { ip: 'www.visaeurope.ch', port: 443, type: 'https' },
    { ip: 'www.visa.com.br', port: 443, type: 'https' },
    { ip: 'www.visasoutheasteurope.com', port: 443, type: 'https' }
];

function parseLinkToSingboxNode(link) {
    try {
        if (link.startsWith('vless://')) {
            const url = new URL(link);
            const name = decodeURIComponent(url.hash.substring(1));
        ...
        // Add Trojan/VMess/SS if needed
    } catch (e) { return null; }
    return null;
}

function generateLocalClashConfig(links, user) {
    const nodes = links.map(link => parseLinkToClashNode(link)).filter(n => n);
    const nodeNames = nodes.map(n => n.name);

    if (nodes.length === 0) return "# No compatible nodes found";
...
function generateLocalSingboxConfig(links, user) {
    const nodes = links.map(link => parseLinkToSingboxNode(link)).filter(n => n);
    const nodeTags = nodes.map(n => n.tag);

    if (nodes.length === 0) return JSON.stringify({ error: "No compatible nodes" });
    ...
    const config = {
        ...
        outbounds: [
            { type: "selector", tag: "select", outbounds: ["auto", ...nodeTags] },
            { type: "urltest", tag: "auto", outbounds: nodeTags, url: "http://www.gstatic.com/generate_204", interval: "3m", tolerance: 50 },
